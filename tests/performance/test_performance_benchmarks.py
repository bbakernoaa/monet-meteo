...
class TestPerformanceBenchmarks:
    ...
    def test_3d_thermodynamic_performance(self, array_size):
        ...
        functions = [
            (
                "saturation_vapor_pressure",
                lambda: saturation_vapor_pressure(temperature),
            ),
            (
                "mixing_ratio",
                lambda: mixing_ratio(
            ...
            (
                "virtual_temperature",
                lambda: virtual_temperature(
                    temperature=temperature, mixing_ratio=humidity * 0.02
                ),
            ),
    ...
    def test_3d_array_efficiency(self):
        ...
        for i in range(nz):
            theta = potential_temperature(
                pressure=pressure[i, :, :], temperature=temperature[i, :, :]
            )
...
class TestOptimizationOpportunities:
    ...
    def test_vectorization_effectiveness(self):
        ...
        start_time = time.time()
        theta_vectorized = potential_temperature(
    ...
    def test_function_call_overhead(self):
        ...
        theta_repeated = potential_temperature(
            pressure=pressure_repeated, temperature=temperature_repeated
        )
    ...
    def test_high_frequency_data_processing(self):
        ...
        for name, n_times in frequencies:
            ...
            for t in range(n_times):
                theta = potential_temperature(
                    pressure=pressures[t], temperature=temperatures[t]
                )
                assert np.all(np.isfinite(theta))
