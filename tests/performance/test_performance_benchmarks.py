...
from monet_meteo.dynamics.dynamic_calculations import divergence, geostrophic_wind, relative_vorticity
from monet_meteo.statistical.statistical_calculations import correlation_coefficient, standard_deviation
...
class TestPerformanceBenchmarks:
    ...
    def test_3d_thermodynamic_performance(self, array_size):
        ...
        functions = [
            ...
            ("mixing_ratio", lambda: mixing_ratio(vapor_pressure=humidity * saturation_vapor_pressure(temperature), pressure=pressure)),
            ("virtual_temperature", lambda: virtual_temperature(temperature=temperature, mixing_ratio=humidity * 0.02)),
...
class TestMemoryEfficiency:
    ...
    def test_large_array_memory_usage(self):
        ...
        mix_ratio = mixing_ratio(vapor_pressure=0.5 * svp, pressure=pressure)
    ...
    def test_3d_array_efficiency(self):
        ...
        start_time = time.time()
        for i in range(nz):
            theta = potential_temperature(pressure=pressure[i, :, :], temperature=temperature[i, :, :])
...
class TestOptimizationOpportunities:
    ...
    def test_vectorization_effectiveness(self):
        ...
        for i in range(size):
            theta_looped[i] = potential_temperature(pressure=pressure[i], temperature=temperature[i])
    ...
    def test_function_call_overhead(self):
        ...
        start_time = time.time()
        theta_repeated = potential_temperature(pressure=pressure_repeated, temperature=temperature_repeated)
...
class TestRealWorldScenarios:
    ...
    def test_global_model_grid_performance(self):
        ...
        for name, nlat, nlon in resolutions:
            ...
            virtual_temperature(temperature=temperature, mixing_ratio=mix_ratio)
    ...
    def test_high_frequency_data_processing(self):
        ...
        for name, n_times in frequencies:
            ...
            for t in range(n_times):
                theta = potential_temperature(pressure=pressures[t], temperature=temperatures[t])
                assert np.all(np.isfinite(theta))
