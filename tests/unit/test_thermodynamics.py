...
class TestPotentialTemperature:
    ...
    def test_potential_temperature_standard_conditions(self, scalar_test_values):
        ...
        pt = potential_temperature(
            pressure=1000.0,  # Use 1000 hPa to match p0
            temperature=scalar_test_values["temperature"],
        )
        # At 1000 hPa and 288.15 K, potential temperature should be equal to temperature
        assert abs(pt - scalar_test_values["temperature"]) < 0.1

    def test_potential_temperature_decreasing_with_height(
        self, sample_pressure, sample_temperature
    ):
        """Test that potential temperature decreases with height for given profiles."""
        pt = potential_temperature(
            pressure=sample_pressure, temperature=sample_temperature
        )
...
class TestVirtualTemperature:
    ...
    def test_virtual_temperature_standard(self, scalar_test_values):
        ...
        assert (
            vt < scalar_test_values["temperature"] * 1.1
        )  # Should not be more than 10% higher
    ...
    def test_virtual_temperature_dry_air(self, scalar_test_values):
        ...
        vt_dry = virtual_temperature(
            temperature=scalar_test_values["temperature"],
            mixing_ratio=0.0001,  # extremely dry air
        )
...
class TestMixingRatio:
    ...
    def test_mixing_ratio_standard(self, scalar_test_values):
        ...
        w = mixing_ratio(
            vapor_pressure=saturation_vapor_pressure(scalar_test_values["temperature"])
            * scalar_test_values["humidity"],
    ...
    def test_mixing_ratio_saturation(self, scalar_test_values):
        ...
        es = saturation_vapor_pressure(scalar_test_values["temperature"])
        ws = mixing_ratio(
            vapor_pressure=es, pressure=scalar_test_values["pressure"] * 100
        )
...
class TestRelativeHumidity:
    ...
    def test_relative_humidity_standard(self, scalar_test_values):
        ...
        rh = relative_humidity(vapor_pressure=e, saturation_vapor_pressure=es)
    ...
    def test_relative_humidity_extreme(self, extreme_values):
        ...
        e_hot = mixing_ratio_to_vapor_pressure(
            mixing_ratio=extreme_values["very_dry"]
            * 0.01,  # Convert percentage to fraction
            pressure=101325.0,
        )
        rh_dry = relative_humidity(
            vapor_pressure=e_hot, saturation_vapor_pressure=es_hot
        )
        assert rh_dry >= 0
        assert rh_dry <= 1.0
        ...
        e_cold = mixing_ratio_to_vapor_pressure(
            mixing_ratio=extreme_values["very_humid"]
            * 0.02,  # Higher mixing ratio for cold air
            pressure=101325.0,
        )
        rh_humid = relative_humidity(
            vapor_pressure=e_cold, saturation_vapor_pressure=es_cold
        )
        assert rh_humid <= 1.0
