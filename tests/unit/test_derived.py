...
from monet_meteo.thermodynamics.thermodynamic_calculations import lifting_condensation_level
...
class TestSeaLevelPressure:
    """Test sea level pressure diagnostic calculation."""

    def test_sea_level_pressure_diagnostic(self):
        """Test sea level pressure reduction."""
        temp_700 = 280.0
        pres_sfc = 95000.0
        geopot_sfc = 500.0 * 9.80665  # 500m elevation

        slp = sea_level_pressure_diagnostic(temp_700, pres_sfc, geopot_sfc)

        # SLP should be greater than surface pressure
        assert slp > pres_sfc
        # For 500m it should be around 1013 hPa
        assert 100000.0 < slp < 102000.0
