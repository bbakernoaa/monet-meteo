...
class TestBulkRichardsonNumber:
    ...
    def test_bulk_richardson_height_dependence(self):
        ...
        ri_small = bulk_richardson_number(
            u_wind, v_wind, potential_temperature, height_small
        )

        # Large height difference
        height_large = np.array([10.0, 200.0])
        ri_large = bulk_richardson_number(
            u_wind, v_wind, potential_temperature, height_large
        )
...
class TestTurbulenceParameters:
    ...
    def test_obukhov_length_standard(self):
        ...
        l_obukhov = obukhov_length(
            friction_velocity, sensible_heat_flux, potential_temperature
        )
...
class TestHeatFluxes:
    ...
    def test_sensible_heat_flux_standard(self):
        ...
        shf = sensible_heat_flux(
            air_temperature, surface_temperature, wind_speed, stability_parameter
        )
    ...
    def test_latent_heat_flux_standard(self):
        ...
        lef = latent_heat_flux(
            vapor_pressure_air, vapor_pressure_surface, aerodynamic_resistance
        )
...
class TestTurbulenceKineticEnergy:
    ...
    def test_turbulence_kinetic_energy_zero_mean(self):
        ...
        # Known calculation
        expected = 0.5 * (
            np.mean(u_prime**2) + np.mean(v_prime**2) + np.mean(w_prime**2)
        )
