...
from monet_meteo.derived.derived_calculations import dewpoint_temperature, heat_index
...
class TestXarrayIntegration:
    ...
    def sample_xarray_data(self):
        ...
        coords = {"pressure": pressure_levels, "latitude": latitudes, "longitude": longitudes}

        temperature = xr.DataArray(
            temperature_data, coords=coords, dims=["pressure", "latitude", "longitude"], attrs={"units": "K", "long_name": "Air Temperature"}
        )
        pressure = xr.DataArray(
            pressure_data, coords=coords, dims=["pressure", "latitude", "longitude"], attrs={"units": "Pa", "long_name": "Atmospheric Pressure"}
        )
        humidity = xr.DataArray(
            humidity_data, coords=coords, dims=["pressure", "latitude", "longitude"], attrs={"units": "1", "long_name": "Relative Humidity"}
        )
        u_wind = xr.DataArray(
            u_wind_data, coords=coords, dims=["pressure", "latitude", "longitude"], attrs={"units": "m/s", "long_name": "Zonal Wind"}
        )
        v_wind = xr.DataArray(
            v_wind_data, coords=coords, dims=["pressure", "latitude", "longitude"], attrs={"units": "m/s", "long_name": "Meridional Wind"}
        )

        return {"temperature": temperature, "pressure": pressure, "humidity": humidity, "u_wind": u_wind, "v_wind": v_wind}
    ...
    def test_virtual_temperature_xarray(self, sample_xarray_data):
        ...
        pressure = sample_xarray_data["pressure"]
        mixing_ratio_val = mixing_ratio(vapor_pressure=humidity * saturation_vapor_pressure(temp), pressure=pressure)
    ...
    def test_relative_humidity_xarray(self, sample_xarray_data):
        ...
        temp = sample_xarray_data["temperature"]
        sample_xarray_data["pressure"]
        mixing_ratio_val = sample_xarray_data["humidity"] * 0.02  # Convert to mixing ratio

        rh = relative_humidity(vapor_pressure=mixing_ratio_val, saturation_vapor_pressure=saturation_vapor_pressure(temp))
...
class TestDaskIntegration:
    ...
    def sample_dask_data(self):
        ...
        return {"temperature": temperature_data, "pressure": pressure_data, "humidity": humidity_data, "u_wind": u_wind_data, "v_wind": v_wind_data}
...
class TestCoordinatePreservation:
    ...
    def test_coordinate_preservation_2d(self):
        ...
        temp = xr.DataArray(temp_data, coords={"latitude": lats, "longitude": lons}, dims=["latitude", "longitude"])
        humidity = xr.DataArray(humidity_data, coords={"latitude": lats, "longitude": lons}, dims=["latitude", "longitude"])
    ...
    def test_attribute_preservation(self):
        ...
        temp = xr.DataArray(temp_data, dims=["level"], attrs={"units": "K", "standard_name": "air_temperature"})
        pressure = xr.DataArray(pressure_data, dims=["level"], attrs={"units": "Pa", "standard_name": "air_pressure"})
...
class TestIntegrationEdgeCases:
    ...
    def test_nan_handling(self):
        ...
        pressure = xr.DataArray([100000.0, 90000.0, 80000.0, 85000.0], coords=coords, dims=["level"])
