...
def xr_convert_pressure(data: xr.DataArray, from_unit: str, to_unit: str) -> xr.DataArray:
    """
...
def xr_convert_temperature(data: xr.DataArray, from_unit: str, to_unit: str) -> xr.DataArray:
    """
...
def xr_calculate_distance(lat1: xr.DataArray, lon1: xr.DataArray, lat2: xr.DataArray, lon2: xr.DataArray, method: str = "haversine") -> xr.DataArray:
    """
    ...
    result = xr.DataArray(distance_m, coords=lat1.coords, dims=lat1.dims, name="distance")

    # Preserve metadata
    result.attrs = {"long_name": "Distance", "units": "m", "standard_name": "distance"}

    return result


def xr_convert_vertical_coord(
    data: xr.DataArray, from_coord: str, to_coord: str, surface_pressure: Optional[xr.DataArray] = None, reference_pressure: float = 1000.0
) -> xr.DataArray:
    """
    ...
    result = xr.DataArray(eval(to_coord), coords=data.coords, dims=data.dims, name=data.name)
...
def add_coordinate_metadata(ds: xr.Dataset, coord_system: str = "cartesian") -> xr.Dataset:
    """
    ...
    if coord_system.lower() == "geographic":
        if "lat" in ds_out.coords:
            ds_out.coords["lat"].attrs.update({"standard_name": "latitude", "long_name": "Latitude", "units": "degrees_north", "axis": "Y"})
        if "lon" in ds_out.coords:
            ds_out.coords["lon"].attrs.update({"standard_name": "longitude", "long_name": "Longitude", "units": "degrees_east", "axis": "X"})
    elif coord_system.lower() == "pressure":
        if "pressure" in ds_out.coords:
            ds_out.coords["pressure"].attrs.update({"standard_name": "air_pressure", "long_name": "Pressure", "units": "Pa", "positive": "down"})
    elif coord_system.lower() == "cartesian":
        if "x" in ds_out.coords:
            ds_out.coords["x"].attrs.update(
                {"standard_name": "projection_x_coordinate", "long_name": "X-coordinate in Cartesian system", "units": "m"}
            )
        if "y" in ds_out.coords:
            ds_out.coords["y"].attrs.update(
                {"standard_name": "projection_y_coordinate", "long_name": "Y-coordinate in Cartesian system", "units": "m"}
            )

    return ds_out


def validate_coordinate_system(ds: xr.Dataset, required_coords: list) -> bool:
    """
