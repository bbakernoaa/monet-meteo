...
def xr_convert_pressure(
    data: xr.DataArray, from_unit: str, to_unit: str
) -> xr.DataArray:
    """
    ...
    """
    res = pressure(data, from_unit, to_unit)
...
def xr_convert_temperature(
    ...
    """
    res = temperature(data, from_unit, to_unit)
    assert isinstance(res, xr.DataArray)
    return res
...
def xr_calculate_distance(
    ...
    result = xr.DataArray(
        distance_m, coords=lat1.coords, dims=lat1.dims, name="distance"
    )
...
def xr_convert_vertical_coord(
    ...
    if from_coord.lower() == "pressure" and to_coord.lower() == "altitude":
        ...
        out_data = (T0 / L) * (1 - (data / p0) ** ((R * L) / (M * g)))
        ...
        out_data = p0 * (1 - (L * data) / T0) ** ((M * g) / (R * L))

    elif from_coord.lower() == "sigma" and to_coord.lower() == "pressure":
        if surface_pressure is None:
            raise ValueError(
                "surface_pressure is required for sigma to pressure conversion"
            )
        out_data = surface_pressure * data

    elif from_coord.lower() == "pressure" and to_coord.lower() == "sigma":
        if surface_pressure is None:
            raise ValueError(
                "surface_pressure is required for pressure to sigma conversion"
            )
        out_data = data / surface_pressure

    else:
        raise NotImplementedError(
            f"Conversion from {from_coord} to {to_coord} not implemented"
        )

    result = xr.DataArray(out_data, coords=data.coords, dims=data.dims, name=data.name)
...
def add_coordinate_metadata(
    ds: xr.Dataset, coord_system: str = "cartesian"
) -> xr.Dataset:
    """
