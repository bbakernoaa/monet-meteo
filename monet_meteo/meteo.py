...
def potential_temperature(pressure, temperature):
    ...
    Uses the Poisson equation to calculation the potential temperature given
    ...
    """
    return temperature * (1000.0 / pressure) ** (R_d / c_pd)
...
def wind_components(speed, wind_direction):
    ...
    Parameters
    ----------
    speed : `pint.Quantity` or `xarray.DataArray`
        The wind speed (magnitude).
    wind_direction : `pint.Quantity` or `xarray.DataArray`
        The wind direction, specified as the direction from which the wind is
        blowing (0-360 degrees), with 360 degrees being North.
    Returns
    -------
    u, v : tuple of `pint.Quantity` or `xarray.DataArray`
        The u and v components of the wind vector, respectively.
    """
    wind_direction_rad = np.deg2rad(wind_direction)
    u = -speed * np.sin(wind_direction_rad)
    v = -speed * np.cos(wind_direction_rad)
    return u, v
...
def relative_humidity_from_dewpoint(temperature, dewpoint):
    ...
    Uses the ratio of vapor pressure to saturation vapor praises to calculate
    relative humidity.
    ...
    temperature : `pint.Quantity` or `xarray.DataArray`
        Air temperature.
    dewpoint : `pint.Quantity` or `xarray.DataArray`
    ...
    `pint.Quantity`
        Relative humidity.
    """
    e = calc_vapor_pressure(dewpoint)
    es = calc_vapor_pressure(temperature)
    return e / es
