...
class AtmosphericProfile:
    ...
    def __post_init__(self):
        ...
        if hasattr(self.pressure, "shape") and hasattr(self.temperature, "shape"):
            if self.pressure.shape != self.temperature.shape:
                raise ValueError(
                    "Pressure and temperature arrays must have the same shape"
                )
        elif hasattr(self.pressure, "__len__") and hasattr(self.temperature, "__len__"):
            if len(self.pressure) != len(self.temperature):
                raise ValueError(
                    "Pressure and temperature arrays must have the same length"
                )
    ...
    def calculate_thermodynamic_properties(self):
        ...
        if (
            self.equivalent_potential_temperature is None
            and self.mixing_ratio is not None
        ):
...
class WindProfile:
    ...
    def __post_init__(self):
        ...
        if not all(shape == shapes[0] for shape in shapes):
            raise ValueError(
                "Height, u_wind, and v_wind arrays must have the same shape"
            )
...
class ThermodynamicProfile:
    ...
    def __post_init__(self):
        ...
        provided = sum(
            x is not None
            for x in [self.dewpoint, self.mixing_ratio, self.relative_humidity]
        )
        if provided == 0:
